...
class _BaseRepository:
    ...
    def generate_id(self) -> str:
        ...
        for r in self.stream():
            try:
                num = int(r[self._id_field].split(TxId.SEP)[1])
                if num > max_num:
                    max_num = num
            except (IndexError, ValueError):
                continue
        return self._id_format.format(max_num + 1)

    def add(self, obj) -> None:
        append_jsonl(self._path, obj.to_dict())

    def find(self, id_val: str) -> dict | None:
        return next((r for r in self.stream() if r[self._id_field] == id_val), None)

    def update(self, id_val: str, fields: dict) -> None:
        records = [{**r, **fields} if r[self._id_field] == id_val else r for r in self.stream()]
        rewrite_jsonl(self._path, records)

    def remove(self, id_val: str) -> None:
        records = [r for r in self.stream() if r[self._id_field] != id_val]
        rewrite_jsonl(self._path, records)
...
class TransactionRepository(_BaseRepository):
    _id_field = TxField.ID
    _id_format = TxId.FORMAT

    def __init__(self, data_dir: Path = DEFAULT_DATA_DIR) -> None:
        super().__init__(data_dir / Files.TRANSACTIONS)


class RecurringRepository(_BaseRepository):
    _id_field = RecurringField.ID
    _id_format = TxId.RX_FORMAT

    def __init__(self, data_dir: Path = DEFAULT_DATA_DIR) -> None:
        super().__init__(data_dir / Files.RECURRING)
